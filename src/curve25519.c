...
static __always_inline u64 get_unaligned_le64(const u8 *p) {
    return ((u64)p[0]) | ((u64)p[1] << 8) | ((u64)p[2] << 16) |
           ((u64)p[3] << 24) | ((u64)p[4] << 32) | ((u64)p[5] << 40) |
           ((u64)p[6] << 48) | ((u64)p[7] << 56);
}

static __always_inline u32 get_unaligned_le32(const u8 *p) {
    return ((u32)p[0]) | ((u32)p[1] << 8) | ((u32)p[2] << 16) |
           ((u32)p[3] << 24);
}

static __always_inline void put_unaligned_le64(u64 v, u8 *p) {
    p[0] = (u8)(v);
    p[1] = (u8)(v >> 8);
    p[2] = (u8)(v >> 16);
    p[3] = (u8)(v >> 24);
...
curve25519_clamp_secret(u8 secret[CURVE25519_KEY_SIZE]) {
    secret[0] &= 248;
    secret[31] &= 127;
    secret[31] |= 64;
}
...
ladder_smallloop_cmult_small_loop(u64 *nq, u64 *nqpq, u64 *nq2, u64 *nqpq2,
                                  u64 *q, u8 byt, u32 i) {
    while (i--) {
        ladder_smallloop_cmult_small_loop_double_step(nq, nqpq, nq2, nqpq2, q,
                                                      byt);
        byt <<= 2;
    }
}
